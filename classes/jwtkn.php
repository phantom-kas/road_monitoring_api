...
class JWT
{
     ...
     */
    public static function decode($jwt, $key, array $allowed_algs = array(),$type = 'a')
    {
        ...
        if ($type == 'a' AND isset($payload->exp) && ($timestamp - static::$leeway) >= $payload->exp) {
            throw new Exception('Expired token');   
        }

        if ($type == 'r' AND isset($payload->exp) && ($timestamp - static::$leeway) >= $payload->exp) {
            throw new Exception('Expired token');   
        }
    ...
    public static function encodeJsonArr($Arr){
        return static::urlsafeB64Encode(static::jsonEncode($Arr));
    }
    /**
